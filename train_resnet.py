...
parser.add_argument('--max_epochs_stop', type=int, default=5, help='max_epochs_stop, defalut 5') 
...
parser.add_argument('--Lambda', type=float, default=1.0, help='weight of fairloss, default 1')
    ...
    for epoch in range(n_epochs):
        ...
        for ii, (data, target, bias) in enumerate(train_loader):
            ...
            if opt.fairloss:
                ...
                loss = criterion(output,target) + opt.Lambda * fair_loss
            else:
...
if opt.fairloss and opt.Lambda == 1.0:
    save_file_name = saved_dir + '/' + opt.img_type + f'_best_model+fairloss.pt'
elif opt.fairloss and opt.Lambda != 1.0:
    save_file_name = saved_dir + '/' + opt.img_type + f'_best_model_' + str(opt.Lambda) + f'_fairloss+lambda.pt'
else:
